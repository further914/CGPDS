function [params, names] = modelExtractParam(model, samd, samr)
...
% 	last update 2008-11-29T09:11:00.000000Z

fhandle = str2func([model.type 'ExtractParam']);%vargplvmExtractParam.m
if nargout < 2
    if(nargin<2)
        params = fhandle(model,samd,samr);
    else
        params = fhandle(model,samd,samr);
    end
else
    [params, namesTemp] = fhandle(model,samd,samr);
end
