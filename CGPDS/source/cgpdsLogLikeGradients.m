function g = cgpdsLogLikeGradients(model ,stepSize, samd, samr)

% cgpdsLogLikeGradients Compute the gradients for the CGPDS.
...
if isfield(model, 'fixW') && model.fixW == 1
    gW = 0*gW;
end
...
g = [gVar gDynKern gInd_v gInd_u gBetaFinal gW(:)' gKern_v gKern_u];   
