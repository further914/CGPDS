function ll = cgpdsLogLikelihood11(model ,stepSize, samd, samr)
...
%             for d = 1:model.d
%                 for j = 1:model.J
%                     ll = ll - 0.5*(tempMatrix.logDetAt_u(d,j) - (-model.k)*log(model.beta));
%                     %if strcmp(model.approx, 'dtcvar')
%                     ll = ll - 0.5*model.beta*model.W(d,j)^2*model.Psi3 + 0.5*model.beta*tempMatrix.TrC_u(d,j);
%                 end
%             end
