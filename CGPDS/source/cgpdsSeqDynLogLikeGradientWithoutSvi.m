function [g, model] = cgpdsSeqDynLogLikeGradientWithoutSvi(model, vardistx, y)

% cgpdsSeqDynLogLikeGradientWithoutSvi Log-likelihood gradient for of a point of the GP-LVM.
...
% Precompuations for the gradients
% gPsi0 = model.beta*(P1TP1_v*model.Psi0'*model.m*[myOb(:,indexPresent); myMs]');
...
% gPsi1_full = zeros(model.k,N+Nstar);
...
for d = 1:length(indexPresent)
    index = indexPresent(d);
    for j = 1:model.J
%         gPsi1_full = gPsi1_full + model.beta * model.W(index,j)^2 * (P1TP1_u{d,j}*model.Psi1'*model.m(:,d)*model.m(:,d)');%N*M
        gPsi1_full = gPsi1_full + model.beta*model.W(index,j)^2*model.m(:,d)*model.m(:,d)'*model.Psi1...
        ...
        gPsi5 = gPsi5 + 0.5*model.beta*model.W(index,j)^2*model.invK_uu...
            ...
            model.Psi1'*model.m(:,d)*model.m(:,d)'*model.Psi1*inv(model.W(index,j)^2*model.Psi5+model.K_uu/model.beta)...
...
% gPsi1 = gPsi1_temp'; % because it is passed to "kernVardistPsi1Gradient" as gPsi1'...
% gPsi5 = (model.beta/2) * model.T1_u; %30*30
...
% gVarmeansLik = 0*gVarmeans0 + gVarmeans1 + 0*gVarmeans2 + gVarmeans3 + 0*gVarmeans4 + gVarmeans5;
% gVarcovsLik = 0*gVarcovs0 + gVarcovs1 + 0*gVarcovs2 + gVarcovs3 + 0*gVarcovs4 + gVarcovs5; 

% gVarmeansLik = gVarmeans0 + gVarmeans2 + gVarmeans4;
% gVarcovsLik = gVarcovs0 + gVarcovs2 + gVarcovs4; 
%
