function [ll, model] = cgpdsSeqDynLogLikelihood(model, vardistx, y,samd,samr)

% cgpdsSeqDynLogLikelihood Log-likelihood of CGPDS for testing data.
...
if ~isempty(indexPresent)   
    ...
    ll2 = -0.5*(lendPresent*(-(model.N-model.k)*log(model.beta) ...
	      ...
	      - (tempMatrix.TrPP_v ...
	      - tempMatrix.TrYYdr)*model.beta);
    ll2 = ll2 - 0.5*model.beta*lendPresent*model.Psi2 + 0.5*lendPresent*model.beta*tempMatrix.TrC_v;
