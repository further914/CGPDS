...
%   end    
  if display > 0
    fprintf(1, 'Cycle %4d  Error %11.6f  Scale %e   Gradient %f\n', j, fnow, beta,gradnew*gradnew');
%     eval([ELBOName '(' num2str(j) '+1,1)=' num2str(fnow) ';']);
%     ELBO(lastIter,1) = fnow;
%     lastIter = lastIter + 1;
  end
  % % % added by Jing Zhao 
  % % % to save the parameters on probable optimal locations
   
  if (success == 1)    
    % Test for termination
    % modified by ZhaoJing
    tmp1=max(abs(alpha*d));
    tmp2= max(abs(fnew-fold)); 
   
%     fprintf(1, 'max(abs(alpha*d))= %11.6f\n  max(abs(fnew-fold))= %11.6f\n', tmp1,tmp2)
    % 2013-06-13
    if (tmp1< options(2) & tmp2< options(3))
      options(8) = fnew;
      return;
      ...
      % modified by ZhaoJing
      tmp3=gradnew*gradnew';
%       fprintf(1, 'gradnew*gradnew= %11.6f \n', tmp3);
      % 2013-06-13
      if (tmp3== 0)
        options(8) = fnew;
        return;
