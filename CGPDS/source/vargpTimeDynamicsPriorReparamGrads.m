function [gVarmeans gVarcovs gDynKern] = vargpTimeDynamicsPriorReparamGrads(dynModel, gVarmeansLik, gVarcovsLik, gInd_v ,gInd_u)
...
%
%	VARGPTIMEDYNAMICSPRIORREPARAMGRADS The function takes a dynamics
%	model structure along with some partial derivatives, amends as
%	appropriate  and calculates the whole gradient for the variational
%	bound. The gradients returned are w.r.t mu_bar and lambda, i.e. the
%	parameters used by the optimiser (the ones introduced by the
%	reparametrization), not the original ones (mu and S). These
%	gradients are obtained in two steps: firstly, another funtion is
%	used to calculate the gradients that correspond only to the
%	likelihood part of the bound and are w.r.t the original parameters
%	mu and S. These quantities are the arguments gVarmenasLik and
%	gVarcovsLik and are calculated using the already implemented code
%	for the static vargplvm, which assumes that the original parameters
%	are not coupled. The current funtions receives these quantities and
%	a) amends with partial derivatives because the variational
%	parameters are coupled via Kt for the dyn. gplvm b) Computes the
%	whole gradients for both parts of the bound, the one corresponding
%	to the likelihood and the one corresponding to the prior. This must
%	be done in a single function because the final formula contains both
%	parts in a nonlinear form. c) Also the kernel hyperparameters for
%	the dynamics kernel are being retured. See the dyn. vargplvm notes
%	for more details.
...
%
%	See also
%	VARGPLVMLOGLIKEGRADIENTS MODELVARPRIORBOUND


%	Copyright (c) 2010-2011 Michalis K. Titsias and Andreas C. Damianou
% 	vargpTimeDynamicsPriorReparamGrads.m SVN version 1767
% 	last update 2011-11-21T23:01:18.201224Z



if isfield(dynModel, 'seq') & ~isempty(dynModel.seq)
    [gVarmeans gVarcovs gDynKern] = vargpTimeDynamicsPriorReparamGradsSeq(dynModel, gVarmeansLik, gVarcovsLik, gInd_v ,gInd_u);
else
    [gVarmeans gVarcovs gDynKern] = vargpTimeDynamicsPriorReparamGrads1(dynModel, gVarmeansLik, gVarcovsLik, gInd_v ,gInd_u);
end
...
function [gVarmeans gVarcovs gDynKern] = vargpTimeDynamicsPriorReparamGradsSeq(dynModel, gVarmeansLik, gVarcovsLik, gInd_v ,gInd_u)
...
if ~isempty(gInd_u)
    gInd_u = reshape(gInd_u, dynModel.N, dynModel.q); %%%%%
end

if ~isempty(gInd_v)
    gInd_v = reshape(gInd_v, dynModel.N, dynModel.q); %%%%%
end
...
for q=1:dynModel.q
    ...
    for i=1:length(dynModel.seq)
        ...
        trGradKL = trGradKL + IBK .*  diagVarcovs * IBK';%%%%why the former is dot product?????????
        ...
        if ~isempty(gInd_u)
            trGradKL = trGradKL + dynModel.vardist.means(seqStart:seqEnd,q) * gInd_u(seqStart:seqEnd,q)';
        end
...
function [gVarmeans gVarcovs gDynKern] = vargpTimeDynamicsPriorReparamGrads1(dynModel, gVarmeansLik, gVarcovsLik, gInd_v ,gInd_u)
