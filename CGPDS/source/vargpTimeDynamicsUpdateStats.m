...
%	dynamics
%
%	VARGPTIMEDYNAMICSUPDATESTATS When the model contains dynamics, this
%	function first updates the dynamics structure with the relevant
%	precomputations (including the calculation of the temporal prior's
%	kernel Kt) and then it calculates the model's original variational
%	means and covariances. This is essential because due to the
%	reparametrization strategy followed (see report) the optimiser and
%	the likelihood-gradient functions operate on the newly introduced
%	parameters which are, however, a function of the original ones. The
%	original ones must, thus, be computed in each cycle. Note that the
%	VARDIST field of the dynamics structure contains the newly
...
% 	vargpTimeDynamicsUpdateStats.m SVN version 1412
...
for q=1:dynModel.q
    ...
    for i=1:length(dynModel.seq)
        ...
        Bt_q = matrixPositiveDefinite( Bt_q );
        Lbt_q = jitChol(Bt_q)';
