...
%	incolved.
%
%	VARGPTIMEDYNAMICSVARPRIORBOUND It takes a dynamic model structure
%	and calculates the term of the bound that only corresponds to the
%	dynamics part, i.e. to the KL term which involves a temporal prior.
%	The bound is a lower Jensens bound for variational approximation.
%	The code supports calculations for when there are multiple sequences
%	to learn from.
...
%     if isempty(find(isinf(LambdaH_q)))&&isempty(find(isnan(LambdaH_q)))
%     else
%         naninfLen = length(find(isinf(LambdaH_q)|isnan(LambdaH_q)));
%         LambdaH_q(isinf(LambdaH_q)|isnan(LambdaH_q)) =  0.5*ones(naninfLen,1) + 0.001*randn(naninfLen,1);
%     end
    
%     if isempty(find(isnan(dynModel.Kt)))&&isempty(find(isinf(dynModel.Kt)))
%     else
%         n = size(dynModel.Kt,1);
%         X = diag(rand(n,1));
%         U = orth(rand(n,n));
%         dynModel.Kt = U'*X*U;
%     end
