function model = vargplvmCreate(q, d, J, Y, options)

% VARGPLVMCREATE Create a GPLVM model with inducing variables.
%
%	Description:
%
%	MODEL = VARGPLVMCREATE(Q, D, Y, OPTIONS, ENABLEDGTN) creates a
%	GP-LVM model with the possibility of using inducing variables to
...
%	   any).
%	  ENABLEDGTN - if set to true, the model will be created in D >> N
...
% 	vargplvmCreate.m SVN version 1720
% 	last update 2011-11-15T01:51:24.076339Z
...
%% modified by JingZhao 2013-12-05
if isfield(options, 'scaleVal')
    model.scale = options.scaleVal;
    model.scale(find(model.scale==0)) = 1;
    if length(options.scaleVal)<model.d
